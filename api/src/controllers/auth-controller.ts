...
export async function googleLoginCallback(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        if (!data.email || !data.name || !data.id) {
            return res.redirect(
                `${process.env.CLIENT_PORT}/login?error=${encodeURIComponent(
                    "Invalid User Data / Data User Tidak Sah"
                )}`
            );
        }
        ...
        if (existingUser) {
            if (existingUser?.accountProvider !== "GOOGLE")
                return res.redirect(
                    `${process.env.CLIENT_PORT}/register?error=${encodeURIComponent(
                        "Email has already been used by another provide / Email sudah terpakai provider lain"
                    )}`
                );
        }
